...
function Results() {
    ...
    if (!results) {
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 font-sans">
                ...
                <p className="text-gray-500 mb-8">Please scan a resume first to view results.</p>
                <button
                    onClick={() => navigate('/')}
    ...
    const scoreColor = isGoodScore ? "#10b981" : currentScore >= 50 ? "#f59e0b" : "#ef4444";
        ...
        <div className="min-h-screen bg-[#F8F9FA] text-gray-900 font-sans flex flex-col">
            ...
            <header className="h-[60px] bg-[#1e1e38] text-white flex items-center px-6 justify-between shrink-0 shadow-md z-20">
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold tracking-wide text-[15px]">ATS Optimizer</span>
                </div>
                <button
                    onClick={() => navigate('/')}
            ...
            <div className="flex flex-1 overflow-hidden h-[calc(100vh-60px)]">
                ...
                <main className="flex-1 overflow-y-auto px-6 py-8 md:px-12 md:py-10">
                    <div className="max-w-4xl mx-auto">
                        ...
                        {/* Greeting Header */}
                        <div className="mb-8">
                            <h1 className="text-3xl font-extrabold text-[#111827] mb-2 tracking-tight">Your Action Plan</h1>
                            <p className="text-gray-500 text-[15px]">Here is your detailed ATS optimization strategy based on the provided Job Description.</p>
                        </div>
                            ...
                            <div className="relative w-36 h-36 flex items-center justify-center shrink-0">
                                <svg className="transform -rotate-90 w-full h-full">
                                    ...
                                    <circle
                                        ...
                                        strokeDashoffset={(2 * Math.PI * 60) - (currentScore / 100) * (2 * Math.PI * 60)}
                                        className="transition-all duration-1000 ease-out stroke-current"
                                        style={{ color: scoreColor }}
                                    />
                                </svg>
                                <div className="absolute flex flex-col items-center">
                                    <span className="text-5xl font-black text-gray-800 tabular-nums leading-none tracking-tight">{currentScore}</span>
                                    <span className="text-[11px] font-bold text-gray-400 mt-1 uppercase tracking-wider">Match</span>
                                </div>
                            </div>

                            <div className="flex-1 text-center md:text-left">
                                <h2 className="text-2xl font-bold text-gray-900 mb-3">Your resume scored {currentScore} out of 100.</h2>
                                ...
                                <div className="bg-[#fff9e6] border border-amber-200 p-4 rounded-lg flex gap-3 text-left">
                                    <svg className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" /></svg>
                                    <p className="text-[13px] text-gray-700 font-medium">
                                        This score is highly strict. Ensure you incorporate the exact changes outlined below to optimize for automated recruiter screenings.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            ...
                            {results.atsKeywordsMissing && results.atsKeywordsMissing.length > 0 && (
                                <div className="bg-white rounded-xl shadow-sm border border-red-200 overflow-hidden">
                                    <div className="px-6 py-4 border-b border-red-100 bg-red-50 flex items-center gap-3">
                                        ...
                                        <h3 className="text-[16px] font-bold text-red-900 tracking-wide">Missing Exact Match Keywords</h3>
                                    </div>
                                    <div className="p-6">
                                        <p className="text-[14px] text-gray-600 mb-4">The ATS scanner will look for these verbatim phrases from the Job Description. You must include them naturally in your experience section.</p>
                                        <div className="flex flex-wrap gap-2">
                                            {results.atsKeywordsMissing.map((kw, i) => (
                                                <span key={i} className="px-3 py-1.5 bg-white border border-gray-200 shadow-sm text-gray-800 text-[13px] font-semibold rounded-md">{kw}</span>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                            ...
                            {results.exactChanges && results.exactChanges.length > 0 && (
                                <div className="bg-white rounded-xl shadow-sm border border-indigo-200 overflow-hidden">
                                    <div className="px-6 py-4 border-b border-indigo-100 bg-indigo-50 flex items-center gap-3">
                                        ...
                                        <h3 className="text-[16px] font-bold text-indigo-900 tracking-wide">Mandatory Resume Fixes</h3>
                                    </div>
                                    <div className="p-6 space-y-4">
                                        {results.exactChanges.map((change, i) => (
                                            <div key={i} className="flex gap-4 p-4 bg-gray-50 rounded-lg border border-gray-100">
                                                <div className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-600 text-sm font-bold flex items-center justify-center shrink-0">{i + 1}</div>
                                                <p className="text-gray-800 text-[15px] leading-relaxed">{change}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {results.suggestedProjects && results.suggestedProjects.length > 0 && (
                                <div className="bg-white rounded-xl shadow-sm border border-emerald-200 overflow-hidden">
                                    <div className="px-6 py-4 border-b border-emerald-100 bg-emerald-50 flex items-center gap-3">
                                        ...
                                        <h3 className="text-[16px] font-bold text-emerald-900 tracking-wide">Recommended Projects to Build</h3>
                                    </div>
                                    <div className="divide-y divide-gray-100">
                                        {results.suggestedProjects.map((proj, i) => (
                                            <div key={i} className="p-6">
                                                <h4 className="text-[16px] font-bold text-gray-900 mb-2">{proj.title}</h4>
                                                <p className="text-gray-600 text-[14px] leading-relaxed">{proj.description}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {results.projectOverviews && (
                                <div className="bg-white rounded-xl shadow-sm border border-purple-200 overflow-hidden mb-8">
                                    <div className="px-6 py-4 border-b border-purple-100 bg-purple-50 flex items-center gap-3">
                                        ...
                                        <h3 className="text-[16px] font-bold text-purple-900 tracking-wide">Interview Prep Tracker</h3>
                                    </div>
                                    <div className="p-6">
                ...
                {/* Right Sidebar - Resume Viewer */}
                <aside className="w-[400px] lg:w-[480px] bg-[#f8fafc] border-l border-gray-200 shrink-0 hidden lg:flex flex-col">
                    <div className="h-[60px] bg-white border-b border-gray-200 flex items-center px-4 shrink-0 gap-2">
                        {results.optimizedResumeText ? (
                            <>
                                <button
                                    onClick={() => setActiveTab('optimized')}
                                    className={`px-4 py-2 text-sm font-bold rounded-md transition-colors ${activeTab === 'optimized' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-500 hover:bg-gray-50'}`}
                                >
                                    Ready Resume (90+)
                                </button>
                                <button
                                    onClick={() => setActiveTab('parsed')}
                                    className={`px-4 py-2 text-sm font-bold rounded-md transition-colors ${activeTab === 'parsed' ? 'bg-indigo-50 text-indigo-700' : 'text-gray-500 hover:bg-gray-50'}`}
                                >
                                    Original Parsed
                                </button>
                            </>
                        ...
                        <div className="ml-auto flex items-center gap-1">
                            ...
                            <button
                                onClick={() => navigator.clipboard.writeText(activeTab === 'optimized' ? results.optimizedResumeText : resumeText)}
                    ...
                    <div className="flex-1 overflow-y-auto p-6">
                        <p className="text-xs text-gray-500 mb-4 font-medium uppercase tracking-wider">
                            {activeTab === 'optimized' ? 'AI-Generated ATS-Optimized Resume' : 'How the ATS sees your file'}
                        </p>
                        <div className="bg-white w-full rounded-lg shadow-sm border border-gray-200 p-6">
                            {activeTab === 'optimized' ? (
                                <div className="text-gray-900 overflow-x-auto">
                                    <div
                                        className="min-w-[700px] p-6 bg-white border border-gray-100 shadow-sm"
                                        style={{
                                        ...
                                        dangerouslySetInnerHTML={{
                                            __html: results.optimizedResumeText.replace(/class="hr"/g, 'style="border-bottom: 1px solid #000; margin-bottom: 4px;"')
                                                .replace(/class="header"/g, 'style="text-align: center; margin-bottom: 8px;"')
                                                .replace(/<h1/g, '<h1 style="font-size: 18px; margin: 0; font-family: Georgia, serif;"')
                                                .replace(/<h2/g, '<h2 style="font-size: 12px; text-transform: uppercase; font-variant: small-caps; margin: 0 0 2px 0;"')
                                                .replace(/class="section"/g, 'style="margin-bottom: 10px;"')
                                                .replace(/class="item"/g, 'style="margin-bottom: 6px;"')
                                                .replace(/class="item-header"/g, 'style="display: flex; justify-content: space-between; align-items: baseline;"')
                                                .replace(/class="item-sub"/g, 'style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 2px;"')
                                                ...
                                                .replace(/<p/g, '<p style="margin: 0 0 2px 0; text-align: justify;"')
                                                .replace(/<li/g, '<li style="margin-bottom: 2px; text-align: justify;"')
                                        }}
                                    />
                                </div>
                            ) : (
                                <div className="font-mono text-[12px] leading-relaxed text-gray-700 whitespace-pre-wrap break-words">
                                    {(resumeText.trim().substring(0, 2000) + (resumeText.length > 2000 ? "\n\n... [Text truncated for preview]" : ""))}
                                </div>
                            )}
                        </div>
                    </div>
